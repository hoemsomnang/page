...
@RestController
@RequestMapping("/api/v1/main-owner")
public class MainOwnerManagementController {

    @Autowired
    ResponseResultMessageService responseResultMessageService;
    @Autowired
    private MainOwnerManagementService mainOwnerManagementService;
    @PostMapping("/update")
    public ResponseEntity<ResponseData> RetrieveListOfficeInformation( Authentication authentication, @RequestBody RequestData<DataUtil> requestData ) throws Exception {
        ...
        try {
            ...
            mainOwnerManagementService.updateMainOwnerInformation(requestData.getBody());
    ...
    public ResponseEntity<ResponseData> registrerOfficeInfo( Authentication authentication, @RequestBody RequestData<DataUtil> requestData ) throws Exception {
        ...
        try {
            ...
            Collection<? extends GrantedAuthority> authorities = authentication.getAuthorities();
            requestData.getBody().setString("userID", userDetails.getUsername() );
            mainOwnerManagementService.registerMainOwnerInformation(requestData.getBody());
